...
namespace ArcMist
{
    namespace Digest
    {
        namespace CRC32
        {
            uint32_t table[256] = {0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419, 0x706af48f, 0xe963a535, 0x9e6495a3, 0x0edb8832, 0x79dcb8a4, 0xe0d5e91e, 0x97d2d988, 0x09b64c2b, 0x7eb17cbd, 0xe7b82d07, 0x90bf1d91,
        ...
        void crc32(InputStream *pInput, unsigned int pInputLength, OutputStream *pOutput)
        {
        ...
        void sha1(InputStream *pInput, unsigned int pInputLength, OutputStream *pOutput) // 160 bit(20 byte) result
        {
            ...
            if(Endian::sSystemType != Endian::LITTLE)
                Endian::swap4((uint8_t *)state, 20);
            for(unsigned int i=0;i<5;i++)
        ...
        namespace RIPEMD160
        {
            ...
            void compress(uint32_t *pResult, uint32_t *pBlock)
            {
            ...
            void finish(uint32_t *pResult, uint32_t *pBlock, unsigned int pBlockLength, uint64_t pTotalLength)
            {
                ...
                {
                    compress(pResult, pBlock); // Compress this block
                    std::memset(pBlock, 0, 64); // Clear the block
                }

                // Put bit length in last 8 bytes of block
                pTotalLength *= 8;
                pBlock[14] = pTotalLength;
                pBlock[15] = pTotalLength >> 32;
                compress(pResult, pBlock);
            }
        }

        void ripEMD160(InputStream *pInput, unsigned int pInputLength, OutputStream *pOutput)
        {
            uint32_t result[5];
            uint32_t block[16];
            unsigned int remaining = pInputLength;
            ...
            while(remaining >= 64)
            {
                ...
                remaining -= 64;
                RIPEMD160::compress(result, block);
            }
            ...
            for(unsigned int i=0;i<5;i++)
                pOutput->writeUnsignedInt(Endian::convert(result[i], Endian::BIG));
        }

        uint32_t SHA256_K[64] = {0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
        ...
        void sha256(InputStream *pInput, unsigned int pInputLength, OutputStream *pOutput) // 256 bit(32 byte) result
        {
            Buffer message;
            message.setInputEndian(Endian::BIG);
            message.setOutputEndian(Endian::BIG);
            ...
            // Make message size congruent to 448(mod 512) bits(56(mod 64) bytes)
            ...
            std::vector<uint32_t> hash, chunk, state;
            unsigned int i, s0, s1, t1, t2;

            hash.push_back(0x6a09e667);
            hash.push_back(0xbb67ae85);
            hash.push_back(0x3c6ef372);
            hash.push_back(0xa54ff53a);
            hash.push_back(0x510e527f);
            hash.push_back(0x9b05688c);
            hash.push_back(0x1f83d9ab);
            hash.push_back(0x5be0cd19);

            while(message.remaining())
            {
                // Get a chunk of 16(32 bit integers) 64 bytes(512 bits)
                chunk.clear();

                while(chunk.size() < 16)
                    chunk.push_back(message.readUnsignedInt());
                ...
                { 
                    ...
                    t1 = state[7] + s1 +((state[4] & state[5]) ^(~state[4] & state[6])) + SHA256_K[i] + chunk[i];
            ...
            for(i=0;i<8;i++)
                pOutput->writeUnsignedInt(hash[i]);
        }
        ...
        void sha512(InputStream *pInput, unsigned int pInputLength, OutputStream *pOutput) // 512 bit(64 byte) result
        {
            Buffer message;
            message.setInputEndian(Endian::BIG);
            message.setOutputEndian(Endian::BIG);
        ...
        bool test()
        {
            ...
            correctDigest.setEndian(Endian::BIG);
             ...
             ******************************************************************************************/
            input.clear();
            input.writeHexAsBinary("");
